...
export const Search = () => {
  ...
  const getData = async () => {
    try {
      const response = await axios.get(
        `https://bd9cd79ca39d8b90.mokky.dev/uzum${search}`
      );
  ...
  return (
    <div className=" max-w-[1300px] m-auto p-[10px] ">
      <div>
      ...
      {loading ? (
        <p className="text-[20px]">Загрузка...</p>
      ) : state.length === 0 ? (
        <p className="text-[20px] mb-[200px] flex items-center justify-center h-[300px]">
          Ничего не найдено
        </p>
      ) : (
        <>
          <h1 className="text-[26px] font-semibold">
            Результаты поиска: {state.length}
          </h1>
          <div className="grid grid-cols-2 gap-[20px] sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 pt-5">
            {state.map((product) => (
              <div
                className="relative max-w-[200px] sm:max-w-[300px]"
                key={product.id}
              >
                <button
