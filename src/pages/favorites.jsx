...
export const Favorites = () => {
  const { like, onDelete } = useContext(LikeContext);
  ...
  return (
    <div className="max-w-[1300px] m-auto">
      ...
      {like.length > 0 && (
        <h1 className="m-[20px] text-[24px] font-bold text-[black] border-b pb-[20px]">
          Мои желания
        </h1>
      )}
      ...
      <div className="flex flex-col justify-center">
        {like.length > 0 ? (
          <div className="grid grid-cols-2  p-[20px] gap-[20px] sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
            {like.map((product) => (
              <div key={product.id} className="relative">
                ...
                >
                  <i class="fa fa-heart text-[24px] text-[#8b00ff]"></i>
                </button>
                <div className="relative" onClick={() => HandleMenu(product)}>
                  <div className="flex flex-col justify-between border relative rounded-[10px] sm:max-w-[300px] max-w-[300px] sm:h-[400px] h-[300px] pb-[10px] hover:shadow-lg transition-all cursor-pointer">
                    <img
                      src={product.image}
                      alt={product.title}
          ...
          <div className="text-center flex flex-col justify-center items-center pt-[60px] gap-[10px] mb-[250px]">
            <img
